        ...
        <div class="w-full overflow-hidden px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
            <div class="pb-4 flex items-center justify-between">
                ...
                @if ($subscriptions->count() > 0)
                    ...
                    <!-- Lien menant à la page d'impression -->
                    <!-- Sur cette page après le choix en doit afficher le nombre d'élève facturé sur la periode -->
                    <a href="{{ route('invoice.list') }}">
                        {{ __('Imprimer facture') }}
                    </a>

                    <!-- Interface ayant la lite des eleves avec filtre sur le code -->
                    <!-- N'afficher que les factures non réglé -->
                    <a href="{{ route('regulation.list') }}"> 
                        {{ __('Réglement') }}
                    </a>
                @endif
            </div>
